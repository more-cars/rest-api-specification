...
Feature: Delete RACE TRACK-hosted-racing-event Relationship
  ...
  Rule: Requests to delete the ›hosted-racing-event‹ relationship are rejected when the provided data is invalid
    ...
    @TEST_MCA-1494 @implemented
    Scenario: Trying to delete a ›hosted-racing-event‹ relationship with invalid RACING EVENT ID
      Given there exists a "RACE TRACK" "Red Bull Ring"
      And "RACING EVENT" "Goodwood Festival of Speed" does NOT exist
